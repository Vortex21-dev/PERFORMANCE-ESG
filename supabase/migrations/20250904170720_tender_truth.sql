/*
  # Fix DELETE statement missing WHERE clause

  1. Problem
    - The refresh_dashboard_performance_data() function contains "DELETE FROM dashboard_performance_data;" without WHERE clause
    - PostgreSQL/Supabase requires WHERE clause for DELETE statements for safety

  2. Solution
    - Add WHERE clause to DELETE statement to make it explicit
    - Use "WHERE true" to delete all rows if that's the intended behavior
*/

-- Fix the refresh_dashboard_performance_data function
...
AS $$
BEGIN
  -- Clear existing data with explicit WHERE clause
  DELETE FROM dashboard_performance_data WHERE true;

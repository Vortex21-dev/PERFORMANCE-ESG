...
BEGIN
  ...
  -- Add sites_count column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'site_indicator_values_consolidated' 
    AND column_name = 'sites_count'
  ) THEN
    ALTER TABLE site_indicator_values_consolidated ADD COLUMN sites_count integer DEFAULT 1;
  END IF;

  -- Add sites_list column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'site_indicator_values_consolidated' 
    AND column_name = 'sites_list'
  ) THEN
    ALTER TABLE site_indicator_values_consolidated ADD COLUMN sites_list text[];
  END IF;
END $$;
...
BEGIN
  ...
  -- FIX: Insert consolidated data with proper aggregation to prevent duplicates
  ...
  SELECT 
    -- FIX: Aggregate data properly to ensure unique rows per month/indicator/site combination
    ...
    -- FIX: Aggregate values properly based on formula for the specific month
    CASE 
      WHEN MAX(i.formule) = 'somme' THEN COALESCE(SUM(iv.value), 0)
      WHEN MAX(i.formule) = 'moyenne' THEN COALESCE(AVG(iv.value), 0)
      WHEN MAX(i.formule) = 'max' THEN COALESCE(MAX(iv.value), 0)
      WHEN MAX(i.formule) = 'min' THEN COALESCE(MIN(iv.value), 0)
    ...
    COUNT(DISTINCT CASE WHEN iv.site_name IS NOT NULL THEN iv.site_name END) as sites_count,
  ...
  -- FIX: GROUP BY all non-aggregate columns including month to prevent repetition
  GROUP BY 
    iv.organization_name,
    iv.business_line_name,
    iv.subsidiary_name,
    ...
    iv.indicator_code,
    iv.year,
    iv.month; -- CRITICAL: Include month in GROUP BY
...
BEGIN
  ...
  -- Check month distribution to ensure no repetition
  SELECT string_agg(DISTINCT month::text, ', ' ORDER BY month::text) INTO month_distribution
  FROM site_indicator_values_consolidated
  WHERE organization_name = 'TestFiliere';
  
  RAISE NOTICE 'Consolidated records created: %, Distinct sites: %, Months: %', 
    record_count, site_count, month_distribution;
END $$;
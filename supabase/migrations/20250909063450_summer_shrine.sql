...
BEGIN
  ...
  SELECT DISTINCT
    ...
    -- Same value for consolidated (site level = raw level)
    CASE 
      WHEN i.formule = 'somme' THEN 
        (SELECT COALESCE(SUM(iv2.value), 0)
         FROM indicator_values iv2
  ...
  -- GROUP BY all non-aggregate columns including month to prevent repetition
  GROUP BY 
    iv.organization_name,
    iv.business_line_name,
    iv.subsidiary_name,
...
BEGIN
  ...
  RAISE NOTICE 'Consolidated records created: %, Distinct sites: %', record_count, site_count;
END $$;
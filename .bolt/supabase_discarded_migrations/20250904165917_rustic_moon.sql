/*
  # Convert dashboard materialized view to editable table

  1. New Tables
    - `dashboard_performance_data` - Editable table replacing materialized view
      - All original columns plus id, created_at, updated_at
      - Unique constraint on (organization_name, process_code, indicator_code, year)
      - RLS enabled for data isolation

  2. Functions
    - `refresh_dashboard_performance_data()` - Refreshes all dashboard data
    - `update_dashboard_performance_data()` - Trigger function for auto-updates
    - `refresh_dashboard_performance_view()` - Compatibility function

  3. Security
    - Enable RLS on dashboard_performance_data table
    - Add policy for organization-based access
...
-- Function to update dashboard data when indicator values change
...
BEGIN
  ...
  -- Refresh data for this organization and year only
  INSERT INTO dashboard_performance_data (
    organization_name,
    process_code,
    indicator_code,

...
class DoubleMaterialityService {
  ...
  async insertDefaultMatrix(organizationName: string): Promise<MatrixRow[]> {
    // Ne plus insérer de données par défaut
    return [];
  }
}
...
export const DoubleMaterialite: React.FC<{
  ...
  const loadMatrixData = async () => {
    ...
    try {
      ...
      if (data.length === 0) {
        // Commencer avec une matrice vide
        setMatrixData([]);
      } else {
        setMatrixData(data);
      }
  ...
  const addMatrixRow = async () => {
    ...
    try {
      const insertedRow = await materialityService.insertMatrixRow(currentOrganization, newRow);
      if (insertedRow) {
